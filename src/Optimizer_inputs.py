...
class RailwayOptimizer:
    def __init__(self, filepath = "D://Project_.xls", opencity = "北京", destination = "桂林"):
        '''
        初始化最优函数类
        Args:
            filepath: 数据路径，默认为D://Project_.xls
            opencity: 起点城市，默认为北京
    ...
    def solve(self, obj_coef = (0.32, 0.4, 0.15, -0.13), print_solution = True):
        ...
        # Obj Func
        gama1, gama2, gama3, gama4 = obj_coef

        z_1 = self.if_rt.prod(self.dists)
        z_2 = self.if_rt.prod(self.Ts) + 2 * (self.if_rt.sum() - 1) + 18 * self.if_x.sum() 
        ...
        self.m.setObjective(gama1*X1 + gama2*X2 + gama3*X3 + gama4*X4, GRB.MINIMIZE)
...
opt = RailwayOptimizer(opencity = opencity, destination = destination)
'''for times in range(1):
    obj_coef_list = np.array([1] + list(np.random.random(size = (3, )) * 3))
    obj_coef_list /= sum(obj_coef_list)
    obj_coef = tuple(obj_coef_list)
    opt.solve(obj_coef = (0.32, 0.4, 0.01, -0.13))
    print(obj_coef)'''
opt.solve()